...
final class DBSQLTypesPanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8637438787361259176L;
    ...
    void loadSettingsFrom(final NodeSettingsRO settings,
        ...
        for (int i = 0; i < specs[0].getNumColumns(); i++) {
            ...
            if (type != null) {
                ...
                textFld.setText(MyDBWriterNodeModel.SQL_TYPE_INTEGER);
            } else if (cspec.getType().isCompatible(DoubleValue.class)) {
                textFld.setText(MyDBWriterNodeModel.SQL_TYPE_DOUBLE);
            } else if (cspec.getType().isCompatible(DateAndTimeValue.class)) {
                textFld.setText(MyDBWriterNodeModel.SQL_TYPE_DATEANDTIME);
            } else {
                textFld.setText(MyDBWriterNodeModel.SQL_TYPE_STRING);
            }
            p.add(label);
            p.add(textFld);
            super.add(p);
