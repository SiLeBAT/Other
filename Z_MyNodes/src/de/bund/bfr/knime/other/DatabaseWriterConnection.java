...
public final class DatabaseWriterConnection {
    ...
    public static final String writeData(
        ...
        synchronized (dbConn.syncConnection(conn)) {
            ...
            final int[] mapping;
            final StringBuilder tableCols = new StringBuilder("(");
            //final int[] theTypes;
            ...
            if (appendData) {
                ...
                if (rs != null) {
                    ...
                    //theTypes = new int[rsmd.getColumnCount()];
                    for (int i = 0; i < mapping.length; i++) {
                        String name = rsmd.getColumnName(i + 1).toLowerCase();
                        //theTypes[i] = rsmd.getColumnType(i);
                        if (i > 1) tableCols.append(", ");
                        if (i > 0) tableCols.append(name);
                    ...
                    //theTypes = new int[spec.getNumColumns()];
                }
            } else {
                mapping = new int[spec.getNumColumns()];
                for (int k = 0; k < mapping.length; k++) {
            ...
            for (int i = 1; i < mapping.length; i++) {
                if (i > 1) wildcard.append(", ");
                wildcard.append(mapping[i] < 0 ? "NULL" : "?");
            }
            wildcard.append(")");
            ...
            final String query = "INSERT INTO "
                + table + " " + tableCols.toString() + " VALUES " + wildcard.toString();
            LOGGER.debug("Executing SQL statement \"" + query + "\"");
            ...
            try {
                ...
                for (RowIterator it = data.iterator(); it.hasNext(); cnt++) {
                    ...
                    for (int i = 0; i < mapping.length; i++) {
                        if (mapping[i] < 0) {
                        	//if (i==0) stmt.setNull(1, Types.INTEGER); // Types.NULL   theTypes[i]
                        	//int[] theTypes = new int[]{Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.BIT,Types.VARCHAR,Types.INTEGER,Types.INTEGER,Types.VARCHAR,Types.VARCHAR,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.DATE,Types.VARCHAR,Types.DATE,Types.VARCHAR,Types.DATE,Types.VARCHAR};
                            //stmt.setNull(dbIdx, theTypes[i]); // Types.NULL   theTypes[i]
                            continue;
                        }
                        dbIdx++;
                        DataColumnSpec cspec = spec.getColumnSpec(mapping[i]);
